...
// Times are 24-hour "HH:mm" strings (native <input type="time"> format).
export const deliverySlotConfigSchema = z.object({
  enabled: z.boolean().default(false),
  deliveryStart: z.string().default(""),
  deliveryEnd: z.string().default(""),
  cutoffTime: z.string().default(""),
});

export const deliverySlotsSchema = z.object({
  lunch: deliverySlotConfigSchema,
  dinner: deliverySlotConfigSchema,
});

// Note: the runtime shape here intentionally matches DeliverySlots /
// DeliverySlotConfig in "@shared/tiffinSlots" (the canonical types for this
// feature) — kept as separate zod schemas here just for form/API validation.
...
export const insertTiffinSchema = z.object({
  ...
  // ✅ UPDATED: generic time slots are only required for Meals now. Tiffin
  // services use the fixed Lunch/Dinner `deliverySlots` config below instead
  // — enforced by the .superRefine() at the bottom of this schema.
  ...
  if (data.serviceType === "meal" && data.slots.length === 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["slots"], message: "Select at least one time slot" });
  }
  if (data.serviceType === "tiffin") {
    ...
    if (!lunchOn && !dinnerOn) {
      ctx.addIssue({
        ...
        path: ["deliverySlots"],
        message: "Turn on Lunch and/or Dinner and set their delivery + cut-off times",
      });
    }
  }
});
...
export const bookingSchema = z.object({
  ...
  // ✅ NEW: Fixed Lunch/Dinner slot picked by the customer — Tiffin module ONLY.
  tiffinSlotType: z.enum(["lunch", "dinner"]).optional(),
  createdAt: z.string(),
});
