...
// ---------------------------------------------------------------------------
// ✅ NEW: Fixed Lunch/Dinner delivery-slot picker — Tiffin module ONLY.
// Customer can only choose one of the seller's configured slots; they can
// never type a custom time. If today's cut-off has already passed for a
// slot, it stays selectable but auto pre-orders for tomorrow instead —
// exactly what "Allow customers to pre-order the next available slot" means.
// ---------------------------------------------------------------------------

function TiffinSlotPicker({
  availableSlots,
  value,
  onChange,
}: {
  availableSlots: ReturnType<typeof getAvailableTiffinSlots>;
  value: TiffinSlotKey | "";
  onChange: (key: TiffinSlotKey) => void;
}) {
  if (availableSlots.length === 0) {
    return (
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700">
        The seller hasn't set up Lunch/Dinner delivery slots for this tiffin yet.
      </div>
    );
  }

  const chosen = availableSlots.find((s) => s.slotKey === value) || null;
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {availableSlots.map((slot) => {
          const selected = value === slot.slotKey;
          return (
            <button
              key={slot.slotKey}
              type="button"
              onClick={() => onChange(slot.slotKey)}
              className={`text-left rounded-lg border p-3 transition-colors ${
                selected ? "border-primary bg-primary/10" : "border-muted-foreground/20 hover:border-muted-foreground/40"
              }`}
            >
              <div className="flex items-center justify-between gap-2">
                <span className="font-semibold text-sm">{slotLabel(slot.slotKey)}</span>
                {slot.todayCutoffPassed && (
                  <Badge variant="outline" className="text-[10px] shrink-0">
                    For tomorrow
                  </Badge>
                )}
              </div>
              <p className="text-xs text-muted-foreground mt-1">{formatSlotWindow(slot.config)}</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">
                Order before {formatTime12h(slot.config.cutoffTime)}
              </p>
            </button>
          );
        })}
      ...
      {chosen &&
        (chosen.todayCutoffPassed ? (
          <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-700">
            <Clock className="mt-0.5 h-4 w-4 shrink-0" />
            <span>Today's slot is closed. Pre-order for tomorrow.</span>
          </div>
        ) : (
          <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 p-3">
            <div className="h-2 w-2 rounded-full bg-green-500" />
            <p className="text-sm font-medium text-green-700">
              Delivering today, {formatSlotWindow(chosen.config)}.
            </p>
          </div>
        ))}
    </div>
...
export default function TiffinDetail() {
  ...
  // ✅ NEW: Fixed Lunch/Dinner slot state — Tiffin module ONLY.
  const [selectedTiffinSlotType, setSelectedTiffinSlotType] = useState<TiffinSlotKey | "">("");
  // Ticks once a minute so an open cut-off ("Today's slot is closed…") is
  // reflected live without requiring a page refresh.
  const [slotTick, setSlotTick] = useState(0);
  useEffect(() => {
    const interval = setInterval(() => setSlotTick((t) => t + 1), 60000);
  ...
  // ✅ NEW: Tiffin module ONLY — fixed Lunch/Dinner slots replace the free
  // date + generic time-slot pickers entirely for tiffin services.
  const isTiffinService = tiffin?.serviceType === "tiffin";
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const availableTiffinSlots = useMemo(
    () => (tiffin ? getAvailableTiffinSlots((tiffin as any).deliverySlots) : []),
    [tiffin, slotTick],
  );
  const selectedSlotAvailability = useMemo(
    () => (tiffin && selectedTiffinSlotType ? getSlotAvailability((tiffin as any).deliverySlots, selectedTiffinSlotType) : null),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [tiffin, selectedTiffinSlotType, slotTick],
  );

  // Keep the delivery date in sync with the chosen slot's cut-off state —
  // customers never type a date for tiffin orders, it's fully derived.
  useEffect(() => {
    if (!isTiffinService || !selectedSlotAvailability) return;
    setSelectedDate(resolveOrderDate(selectedSlotAvailability.nextAvailableDay));
  }, [isTiffinService, selectedSlotAvailability]);
  ...
  const handleBookNow = () => {
    ...
    if (isTiffinService) {
      if (!selectedTiffinSlotType) {
        toast({ title: "Select Lunch or Dinner", variant: "destructive" });
        return;
      }
      if (!selectedSlotAvailability?.enabled) {
        toast({ title: "This slot isn't available", variant: "destructive" });
        return;
      }
    } else if (requiresTimeSlot && !selectedSlot) {
  ...
  const confirmBooking = () => {
    ...
    const finalSlot = isTiffinService
      ? selectedSlotAvailability
        ? `${slotLabel(selectedSlotAvailability.slotKey)} (${formatSlotWindow(selectedSlotAvailability.config)})`
        : ""
      : selectedSlot === "Now"
    ...
    const bookingData: BookingData = {
      ...
      ...(isTiffinService && selectedTiffinSlotType && { tiffinSlotType: selectedTiffinSlotType }),
              ...
              <CardContent className="space-y-4 p-4">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {tiffin.isAvailable === false && (
                      <Badge className="bg-red-600 hover:bg-red-600 text-white shrink-0">Out of Stock</Badge>
                    )}
                  </div>
                ...
                {tiffin.isAvailable === false && (
                  <div className="flex items-center gap-2 rounded-lg border border-red-200 bg-red-50 p-3 text-sm text-red-700">
                    ...
                    <span>The seller has temporarily marked this item unavailable. Please check back later.</span>
                  </div>
                )}
                ...
                {isTiffinService ? (
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="font-medium">Delivery Slots</p>
                    </div>
                    {availableTiffinSlots.length === 0 ? (
                      <p className="text-sm text-muted-foreground">No delivery slots configured yet.</p>
                    ) : (
                      <div className="flex flex-wrap gap-2">
                        {availableTiffinSlots.map((slot) => (
                          <Badge key={slot.slotKey} variant="secondary" className="text-xs font-normal">
                            {slotLabel(slot.slotKey)}: {formatSlotWindow(slot.config)} · cut-off{" "}
                            {formatTime12h(slot.config.cutoffTime)}
                          </Badge>
                        ))}
                      </div>
                    )}
              ...
              <CardContent className="space-y-5">
                {!isAuthenticated ? (
                  ...
                  <>
                    ...
                    {isTiffinService ? (
                      <TiffinSlotPicker
                        availableSlots={availableTiffinSlots}
                        value={selectedTiffinSlotType}
                        onChange={setSelectedTiffinSlotType}
                      />
                    ) : (
                      <>
                        <div className="space-y-2">
                        ...
                        {requiresTimeSlot ? (
                          <div className="space-y-2">
                            ...
                            <Button
                              ...
                              onClick={handleQuickOrderNow}
                              disabled={tiffin.isAvailable === false}
                            >
                    ...
                    <Button
                      ...
                      disabled={bookingMutation.isPending || tiffin.isAvailable === false}
                    >
                      {tiffin.isAvailable === false
                        ? "Out of Stock"
