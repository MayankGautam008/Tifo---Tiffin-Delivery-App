...
export function useRealtimeOrders() {
  ...
  useEffect(() => {
    ...
    // A seller flipped an item in/out of stock → refresh tiffin listings
    // and detail pages everywhere, instantly, for every logged-in user.
    const handleAvailabilityUpdated = () => {
      invalidateByPrefix(["/api/tiffins", "/api/seller/tiffins"]);
    };
    ...
    socket.on("tiffin:availability-updated", handleAvailabilityUpdated);
